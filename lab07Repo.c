...
void imprimeNo(No* no){
  if(no != NULL){
    printf("\n");
    printf("%s la", no->pasta);
    printf("[%s]\n", no->pasta.programa);

    if(no->dir != NULL)
      printf("%s_dir: %s [%s]\n", no->pasta.nome, no->dir->pasta.nome, no->dir->pasta.programa);

    if(no->esq != NULL)
      printf("%s_esq: %s [%s]\n", no->pasta.nome, no->esq->pasta.nome, no->esq->pasta.programa);
  }
...
No* restaura(No* no, char** pre, char** in, int* preTamanho, int inTamanho){
  ...
  if(indexRaiz > 0){
    ...
    if(no->esq != NULL){
      no->esq->pasta.nome = malloc(sizeof(char*));
      strcpy(no->esq->pasta.nome, no->pasta.programa);
      strcat(no->esq->pasta.nome, "_esq");

      printf("no->esq->pasta->nome: %s\n", no->esq->pasta.nome);
      no->esq->pai = no;
    }
  ...
  if(tamanhoInDir > 0){
    ...
    if(no->dir != NULL){
      no->dir->pasta.nome = malloc(sizeof(char*));
      strcpy(no->dir->pasta.nome, no->pasta.programa);
      strcat(no->dir->pasta.nome, "_dir");

      printf("no->dir->pasta->nome: %s\n", no->dir->pasta.nome);
      no->dir->pai = no;
    }
  }
...
int main(){
  ...
  arvore.raiz = restaura(arvore.raiz, arvore.sementePreOrder, arvore.sementeInOrder, &arvore.tamanhoPreOrder, arvore.tamanhoInOrder);
